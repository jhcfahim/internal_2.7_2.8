...
def create_budget_file():
  ...
  while True:
    file_name = input(
      "\n\033[32mWhat would you like the file name to be? \033[0m")
    ...
    if not file_exists(file_name):
      ...
      print(
        "\033[31mA file already exists with that name. Please choose a different name.\033[0m"
      )
  ...
  with open(file_name, "w") as f:
    ...
    while True:
      income_before_tax = input(
        "\n\033[32mEnter your monthly income before tax $: \033[0m")
...
Budget: 200
    ...
    # If user doesn't need help, ask for the budget
      ...
      budget = check_num("\033[32mAdd a budget (enter a number) $: \033[0m")

      # Check if the budget is more than discretionary income
      if budget > income_after_tax:
        print(
          "\033[31mYour budget is bigger than available income. Please enter a smaller budget.\033[0m"
        )
...
def edit_budget_category():
  ...
  with open(file_edit, "r+") as f:
    ...
    while not category_exists:
      category_to_edit = input(
        "\033[32mWhich category do you want to edit? \033[0m")
    ...
    while action not in ["value", "name"]:
      action = input(
        "\033[32mDo you want to change the name or value of the category? \033[0m"
      ).lower()
    ...
    if action == "value":
      new_value = input(
        "\033[32mWhat is the new value for the category? \033[0m")
      category_value = new_value
    # If user wants to change the name, prompt for new name and update.
    elif action == "name":
      new_name = input(
        "\033[32mWhat is the new name for the category? \033[0m")
